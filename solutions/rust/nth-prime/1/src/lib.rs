...
fn is_prime(x: u32) -> bool {
    match x {
        ...
        _ if x % 2 == 0 => false,
        _ => {
            let sqrt_x = f64::from(x).sqrt().floor() as u32;  // Calculate the lowest value where a divider can be found
            (2..=sqrt_x).all(|d| x % d != 0)                // Check this range of values if a divider exists
        }
    }
}
