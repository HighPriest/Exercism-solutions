...
impl<T: ToString> Luhn for T {
    fn valid_luhn(&self) -> bool {
        ...
        let code_string = self.to_string();
        let code = code_string.chars().filter(|&c| c != ' ').rev();
        // Create an iterator over the original string, without making a copy of it.

        if code
            .clone() // With clone, we are creating another iterator with parameters of the previous one. This seems great, because we are not modifying the original string, which is of arbitrary length.
            .any(|c| !c.is_ascii_digit())
        {
            // Check if there are no random non-digit characters in our code
            return false;
        }
        ...
        + code.skip(1).step_by(2)// Iterate over every even value. These require manipulation
            ...
            == 0 // Check if the last digit is 0 (by modulo 10) & immediately return result
    }
}
