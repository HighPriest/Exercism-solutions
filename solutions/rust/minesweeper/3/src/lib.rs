...
pub fn annotate(minefield: &[&str]) -> Vec<String> {
    ...
    minefield.iter().enumerate().for_each(|(y, row)| {
        ...
        row.chars().enumerate().for_each(|(x, field)| {
            // Count bombs or return bomb character
            annotated_field[y][x] = count_bombs(minefield, field, y as isize, x as isize).unwrap_or(' ');
        });
    });
...
#[must_use]
fn count_bombs(minefield: &[&str], field: char, y: isize, x: isize) -> Option<char> {
    if field == '*' {
        ...
        let mine_count = NEIGHBORS
            ...
            .filter(|&&(ny, nx)| {
                ...
                (y + ny) >= 0 && (y+ ny) < minefield.len() as isize &&
                // Check if we are within x bounds
                (x + nx) >= 0 && (x + nx) < minefield[0].len() as isize &&
                // Check if the neighboring cell contains a mine in the original field
                minefield[(y+ny) as usize].chars().nth((x+nx) as usize) == Some('*')
            })
